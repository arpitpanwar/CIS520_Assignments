...
Z = zeros([size(Y,1),size(unique(Y))]);
...
if depth == depth_limit || length(unique(Z)) == 1 || numel(Z) <= 1 || numel(colidx) == 0
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if numel(Z) == 0
...
fprintf('depth %d [%d]: Split on feature %d <= %.2f w/ IG = %.2g (L/R = %d/%d)\n', ...
    depth, numel(Z), node.fidx, node.fval, max_ig, numel(leftidx), numel(rightidx));
